...
export default function ProductCard({ product }: { product: Product }) {
  return (
    <motion.div
      ...
      className="group bg-card-bg dark:bg-neutral-900 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl border border-white/10 transition-all text-white p-1 md:p-6"
    >
      <Link
        href={`/products/${product.id}`}
        className="block relative aspect-square overflow-hidden bg-muted/10"
      >
        <Image
          src={product.image}
          alt={product.name}
      ...
      <div className="pt-2 md:pt-6">
        ...
        <div className="flex items-center justify-between mt-auto">
          ...
          <Link
            href={`/products/${product.id}`}
            className="text-[10px] md:text-sm font-bold text-primary dark:text-primary-light hover:text-primary-light hover:underline underline-offset-4 transition-colors"
          >
            View Details
          </Link>
        </div>
